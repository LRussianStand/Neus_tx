# -*- coding: utf-8 -*-
""" 
@Time    : 2022/9/19 15:35
@Author  : HCF
@FileName: neus_mvs.py
@SoftWare: PyCharm
"""
...
from models.data_MVS import Dataset
...
class Runner:
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            rays_o, rays_d, true_rgb, mask, ray_len_mvs, ray_len_mvs_mask, consistency, consistency_mask \
                = data[:, :3], data[:, 3: 6], data[:, 6: 9], data[:, 9: 10], \
                  data[:, 10: 11], data[:, 11: 12], data[:, 12: 13], data[:, 13: 14]
            near, far = self.dataset.near_far_from_sphere(rays_o, rays_d)

            ray_len_mvs_mask = torch.logical_and(ray_len_mvs >= near, ray_len_mvs <= far) * ray_len_mvs_mask
